...
def _process_frame_grayscale(frame):
    obs = frame[:, :, :].astype(np.float)/255.0
    obs = resize(obs, (SCREEN, SCREEN))
    obs = ((1.0 - obs) * 255).round().astype(np.uint8)
...
class AtariTest(AtariEnv):
    def __init__(self,gname, full_episode=False, full_action = True):#, gname = current_game):
        self.gname = gname.env_name
        super(AtariTest, self).__init__(game=self.gname,obs_type="image",full_action_space=full_action)
        ...
        correctmodel = self.gname+'/z'+str(self.z_size)+'/encoder'
    ...
    def reset(self):
        ...
        newscreen = newscreen.reshape(1, SCREEN, SCREEN, CH)
    ...
    def render(self, mode='human', close=False):
        ...
        return super(AtariTest, self).render(mode=mode)
    
    def seed(self,seed = None):
        return super(AtariTest,self).seed(seed=seed)
    ...
    def step(self, action):
        ...
        obs, reward, done , _ = super(AtariTest,self).step(action)
        ...
        result = result.reshape(1, SCREEN, SCREEN, CH)
