...
# -- 'Weight Agnostic Network' evaluation -------------------------------- -- #
  ...
  def getFitness(self, wVec, aVec, hyp, \
    ...
    total_steps = np.empty((nRep,nVals))
    for iRep in range(nRep):
      for iVal in range(nVals):
        wMat = self.setWeights(wVec,wVals[iVal])
        #print("size of wMat",sys.getsizeof(wMat)/1e06,'MB')
        if seed == -1:
          reward[iRep,iVal],total_steps[iRep,iVal] = self.testInd(wMat, aVec, seed=seed,view=view)
        else:
          reward[iRep,iVal],total_steps[iRep,iVal] = self.testInd(wMat, aVec, seed=seed+iRep,view=view)
    #print(total_steps)      
          
    if returnVals is True:
      return np.mean(reward,axis=0), wVals
    return np.mean(reward,axis=0),np.sum(total_steps,axis=0)
