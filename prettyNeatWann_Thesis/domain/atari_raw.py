...
class AtariRaw(AtariEnv):
    def __init__(self,gname, full_episode=False,full_action = False):#, gname = current_game):
        self.gname = gname.env_name
        super(AtariRaw, self).__init__(game=self.gname,obs_type="image",full_action_space=full_action)
    ...
    def reset(self):
        ...
        newscreen = np.copy(_process_frame(super(AtariRaw, self).reset())).astype(np.float)/255.0
        state = newscreen.reshape(SCREEN*SCREEN*CHANNELS)
        #state,_,_ = self.vae(newscreen)
        #state = np.copy(super(AtariRaw,self).reset()).astype(np.float)/255.0
    ...
    def render(self, mode='human', close=False):
        ...
        return super(AtariRaw, self).render(mode=mode)
    
    def seed(self,seed = None):
        return super(AtariRaw,self).seed(seed=seed)
    ...
    def step(self, action):
        ...
        obs, reward, done, _ = super(AtariRaw,self).step(action)
        #print("USING RAM")
        z = np.copy(_process_frame(obs)).astype(np.float)/255.0
        z = np.reshape(z,SCREEN*SCREEN*CHANNELS)
