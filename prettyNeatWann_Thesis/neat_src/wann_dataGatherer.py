...
class WannDataGatherer():
  ...
  def display(self):
    ...
    if os.path.isfile(self.filename+'.csv'):
        ...
        column_names = ['Elite Fit','Best Fit', 'Peak fit']
        with open(self.filename+'.csv', 'w') as f:
            writer = csv.writer(f) #this is the writer object
            writer.writerow(column_names) # this will list out the names of the columns which are always the first entrries
            
    with open(self.filename+'.csv', 'a') as f:
      writer = csv.writer(f) #this is the writer object
      writer.writerow([self.fit_max[-1],self.fit_top[-1],self.fit_peak[-1]]) # this will list out the names of the columns which are always the first entrries
