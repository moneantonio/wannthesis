...
def act(weights, aVec, nInput, nOutput, inPattern):
  ...
  if np.ndim(weights) < 2:
      nNodes = int(np.sqrt(np.shape(weights)))
      wMat = np.reshape(weights, (nNodes, nNodes))
  else:
      nNodes = np.shape(weights)[0]
      #print(nNodes) 168
      wMat = weights #(168,168)
  wMat[np.isnan(wMat)]=0
  #print(type(inPattern)) #ndarray
  # Vectorize input
  ...
  nodeAct  = np.zeros((nSamples,3,nNodes))
  nodeAct[:,:,0] = 1 # Bias activation
  #print(nodeAct.shape) #210
  #print(inPattern.shape) #(210, 160, 3)
  inPattern = np.reshape(inPattern,(inPattern.shape[0],inPattern.shape[2],inPattern.shape[1]))
...
def softmax(x):
    ...
    """   
    #print(x.shape) #(210,3,168) while x.T is (168,3,210)
    temp_x = x
    temp_x = np.reshape(temp_x,(temp_x.shape[2],temp_x.shape[0],temp_x.shape[1])) #(168,210,3)
    #print(np.max(x,axis=1).shape) #(210,168)
    if x.ndim == 1:
      e_x = np.exp(x - np.max(x))
      return e_x / e_x.sum(axis=0)
    else:
      #temp_x = np.reshape(x,(x.shape[0],x.shape[2],x.shape[1]))
      e_x = np.exp(temp_x.T - np.max(x,axis=1)) #basis (168,3,210) (210,168), with temp (3,210,168)(210,168)
      #print("SHAPE OUTPUT SOFTMAX",((e_x / e_x.sum(axis=0)).T).shape)
