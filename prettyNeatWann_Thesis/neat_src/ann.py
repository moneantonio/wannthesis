...
def act(weights, aVec, nInput, nOutput, inPattern):
  ...
  #print("LEN WEIGHTS",len(weights)) # = nNodes = 20 for racing, 35 for both for Atari
  if np.ndim(weights) < 2:
      ...
      #print("nNodes",nNodes)
      wMat = weights
      #print("SHAPE wMat",wMat.shape) #wMat(20,20) for racing, (35,35) for Atari
  wMat[np.isnan(wMat)]=0
  ...
  if np.ndim(inPattern) > 1:
      ...
      nSamples = 1
  #print("nSAMPLES",nSamples) # nSamples = 1 in vaeracing , 210 in Atari
  # Run input pattern through ANN    
  nodeAct  = np.zeros((nSamples,nNodes))
  nodeAct[:,0] = 1 # Bias activation
  #print("VAERACING INPATTERN SHAPE",inPattern.shape) #(16,)
  #print("INPATTERN SHAPE",inPattern.shape) #(128,)
  #inPattern = np.resize(inPattern,(16,))
  #print("INPATTERN",inPattern)
  #print("nINPUT",nInput) #16
